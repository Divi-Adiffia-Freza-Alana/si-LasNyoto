...
class PurchasingController extends Controller
{
    ...
    public function selectBagpurchasing (Request $request)
    {
        $purchasing = [];
        if($request->has('q')){
            $search = $request->q;
            $purchasing =Purchasing::select("id", "nama")
                    ->where('nama', 'LIKE', "%$search%")
                    ->get();
        }else{ 
            $purchasing =Purchasing::select("id", "nama")->orderBy('id')->get(10);
        }
    ...
    public function index(Request $request){
        ...
        if ($request->ajax()) {
           // $kurir = Kurir::query('');
           $purchasing = Purchasing::with('user');
                  ...
                  ->editColumn('user.name', function($data){
                   ...
                   /* ->editColumn('status_kehadiran', function($data){
                    ...
                    ->addColumn('action', function($row){
                           $btn = '<a class="btn btn-primary" href="/purchasing-edit/'.(isset($row->id)?$row->id:"").'" style="color:#ffff;display:inline-block;" ><i class="fa-solid fa-pen-to-square"></i> </a>
                                   <a class="btn btn-danger" href="/purchasing-delete/'.(isset($row->id)?$row->id:"").'" style="color:#ffff;display:inline-block;" ><i class="fa-solid fa-trash"></i></a>';
                            return $btn;
                    })
                    ->rawColumns(['action'])
                    ->make(true);
    ...
    public function add(){
        ...
        return view('purchasing.add_purchasing');
    ...
    public function edit($id){
        ...
        $purchasingdata = Purchasing::query()->get()->find($id);
       // $kandangdata = Kandang::with('keeperKandang')->get()->find($id);
        //dd($keeperdata);
        return view('purchasing.add_purchasing',['data' =>$purchasingdata]);
    ...
    {   
            ...
            if($request->id == NULL || $request->id == "" ){
                ...
                $purchasing = Purchasing::create([
                 ...
                 Session::flash('message', 'Tambah Data Staf bahan baku Berhasil');
              }
     else{
        //dd($namefile);
        ...
        Purchasing::updateOrCreate(
           ...
           /*  $namefile='';
             ...
             Session::flash('status', 'success');
             Session::flash('message', 'Edit Data Staf Bahan Baku Berhasil');
    ...
    public function delete($id){
        ...
        $delete = Purchasing::findorFail($id);
        $delete->delete();

        /*$deleteKeeperfoto = Keeper_foto::findorFail($id);
        $deleteKeeperfoto->delete();*/
        Session::flash('status', 'success');
        Session::flash('message', 'Delete Data Staf Bahan Baku Berhasil');
